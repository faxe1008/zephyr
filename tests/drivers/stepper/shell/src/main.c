...
ZTEST(stepper_shell, test_stepper_set_step_interval)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper set_step_interval " FAKE_STEPPER_NAME " 200");

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_set_step_interval_fake, err);
	zassert_equal(fake_stepper_set_step_interval_fake.arg1_val, 200,

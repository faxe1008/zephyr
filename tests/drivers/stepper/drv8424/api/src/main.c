...
ZTEST_F(drv8424_api, test_is_not_moving_when_disabled)
{
	...
	(void)stepper_set_step_interval(fixture->dev, 20000);
...
ZTEST_F(drv8424_api, test_move_to_zero_step_interval)
{
	int32_t pos = 100;
	int32_t ret = 0;
...
{
	uint64_t step_interval = 20000;
	int32_t steps = 0;

	(void)stepper_enable(fixture->dev, true);
	(void)stepper_set_step_interval(fixture->dev, step_interval);

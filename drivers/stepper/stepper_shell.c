...
static int cmd_stepper_set_step_interval(const struct shell *sh, size_t argc, char **argv)
{
	const struct device *dev;
	int err = 0;
	...
	err = stepper_set_step_interval(dev, step_interval);
	if (err) {
		shell_error(sh, "Error: %d", err);
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(set_step_interval, &dsub_pos_stepper_motor_name,
		      "<device> <step_interval_us>", cmd_stepper_set_step_interval, 3, 0),

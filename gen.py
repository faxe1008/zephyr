...
def write_c_header(output_path, bitmap_width, height, bytes_per_row, glyphs, source_font, size, avg_width):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"#define FONT_BITMAP_WIDTH {bitmap_width}\n")
        f.write(f"#define FONT_BYTES_PER_ROW {bytes_per_row}\n")
        f.write(f"#define FONT_FIRST_CHAR 32\n")
        f.write(f"#define FONT_LAST_CHAR 127\n")
        f.write(f"#define FONT_CHAR_COUNT {len(glyphs)}\n")
        f.write(f"#define FONT_DEFAULT_WIDTH {int(avg_width + 0.5)}\n")
        f.write(f"#define FONT_CHAR_SPACING 1\n\n")

        # Structure definition
        f.write("typedef struct {\n")
        f.write("    uint32_t unicode;\n")
        f.write("    uint8_t width;        // Actual character width\n")
        f.write(f"    uint8_t bitmap[{height * bytes_per_row}];  // Bitmap data ({bytes_per_row} bytes per row)\n")
        f.write("} FontGlyph;\n\n")

        # Font data
        f.write("static const FontGlyph font_data[] = {\n")

        for i, (unicode, width, bitmap) in enumerate(glyphs):
            char_name = format_char_name(unicode)

            f.write(f"    {{{unicode:3d}, {width:2d}, {{")

            # Format bitmap data nicely
            hex_values = [f"0x{b:02X}" for b in bitmap]

            if len(hex_values) <= 8:
                # Single line for small bitmaps
                f.write(", ".join(hex_values))
            else:
                # Multi-line for larger bitmaps
                f.write("\n")
                for j in range(0, len(hex_values), 8):
                    chunk = hex_values[j:j+8]
                    f.write("        " + ", ".join(chunk))
                    if j + 8 < len(hex_values):
                        f.write(",")
                    f.write("\n")
                f.write("    ")

            comma = "," if i < len(glyphs) - 1 else ""
            f.write(f"}}}}{comma} // {char_name} (width: {width})\n")

 ...
 *
 * @see stepper_set_step_interval() for details.
 */
...
__subsystem struct stepper_driver_api {
	...
	stepper_set_step_interval_t set_step_interval;
 ...
 * set_step_interval and move is required to set the motor into motion.
 *
 * @param dev pointer to the stepper motor controller instance
 * @param step_interval_us time interval between steps in microseconds
 *
 * @retval -EIO General input / output error
 * @retval -EINVAL If the requested step interval is not supported
 * @retval 0 Success
 */
__syscall int stepper_set_step_interval(const struct device *dev, uint64_t step_interval_us);
						   ...
						   const uint64_t step_interval_us)
{
	...
	return api->set_step_interval(dev, step_interval_us);
}
